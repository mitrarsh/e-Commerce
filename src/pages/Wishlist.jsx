  ...
  const handleAddAllToCart = () => {
    if (likedItems) {
      for (let i of likedItems) {
        setCartItems((prev) =>
          prev.some((item) => item.id === i.id) ? prev : [...prev, i]
        );
      }
      setLikedItems([]);
    }
  ...
  return (
    <div className="wishlist-page">
      <div className="wishlist-header">
        ...
        <button className="wishlist-btn" onClick={handleAddAllToCart}>
          Move All to Cart
        </button>
      </div>
      {likedItems.length === 0 ? (
        <p className="emty-msg">Your wishlist is empty.</p>
      ) : (
        <motion.div
              ...
              <div className="item-card">
                <div className="cover">
                  ...
                  <Link to={`/products/${item.id}`}>
                    ...
                    <div className="cover-tag tag-discount">
                      -{item.discount}
                    </div>
